...
async function fetchStockList(sosok, pages = 4) {
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    // 2026-08-28: res.ok 체크가 없어서, 접근 차단(egress allowlist 403 등) 응답 본문을
    // 그대로 EUC-KR/HTML로 파싱 시도 -> 매칭되는 행이 없어 "정상인데 빈 배열"로 둔갑하는
    // 사고가 있었음(22일간 KR 스캔이 전부 무효였는데도 에러 없이 조용히 넘어감).
    // 이제 비정상 응답이면 명확히 에러를 던져서 상위(daily_update.js)의 에러 격리 로직이
    // 실제로 잡아낼 수 있게 함.
    if (!res.ok) {
      const bodyPreview = (await res.text()).slice(0, 200);
      throw new Error(`네이버 종목리스트 응답 오류 (sosok=${sosok}, page=${page}): 상태코드 ${res.status} - ${bodyPreview}`);
    }
