...
function runBreakoutBacktest({ dates, opens, highs, lows, closes, volumes, indexCloses, lookback }) {
  ...
  // 트렌드템플릿(200일선+1개월)·52주저점상승 계산에 필요한 기간까지 감안해 최소 lookback 결정
  const trendTemplateNeed = CONFIG.trendMaLong + CONFIG.trendMaLongRisingLookback;
  const minLookback = Math.max(effectiveLookback, CONFIG.fiftyTwoWeekLowLookback, trendTemplateNeed) + 1;
  const trades = [];
  ...
  for (let i = minLookback; i < n - 1; i++) {
    ...
    if (!inPosition) {
      ...
      const lowRise = riseFromFiftyTwoWeekLow(lowsSlice, closesSlice);
      const vcp = checkVCP(closesSlice, volumesSlice);
      ...
      const lowRiseOk = lowRise !== null && lowRise >= CONFIG.minRiseFromLowPct;
      const vcpOk = vcp.vcpOk === true;

      if (regimeOk && breakout.isBreakout && volumeOk && nearHighOk && trendTemplateOk && lowRiseOk && vcpOk) {
        // 다음날 시가에 진입한다고 가정
        entry = {
          entryIndex: i + 1,
          entryDate: dates[i + 1],
