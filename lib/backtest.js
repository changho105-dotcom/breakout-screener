...
function runBreakoutBacktest({ dates, opens, highs, lows, closes, volumes, indexCloses, lookback }) {
  ...
  const minLookback = effectiveLookback + 1; // 신고가·전고점 돌파 계산에 필요한 최소 기간
  const trades = [];
  ...
  for (let i = minLookback; i < n - 1; i++) {
    ...
    if (!inPosition) {
      const closesSliceForBreakout = closesSlice;
      const lowsSlice = lows.slice(0, i + 1);
      const breakout = checkBreakout(highsSlice, closesSliceForBreakout, lowsSlice, effectiveLookback);
