...
function runBreakoutBacktest({ dates, opens, highs, lows, closes, volumes, indexCloses }) {
  ...
  for (let i = minLookback; i < n - 1; i++) {
    ...
    if (!inPosition) {
      ...
      const rs = relativeStrength(closesSlice, indexSlice);

      const regimeOk = regime === 'STRONG_UP' || regime === 'WEAK_UP';
      const volumeOk = volRatio !== null && volRatio >= 1.5;
      const rsOk = rs !== null && rs > 0;

      if (regimeOk && breakout.isBreakout && volumeOk && rsOk) {
        // 다음날 시가에 진입한다고 가정
        entry = {
          entryIndex: i + 1,
          entryDate: dates[i + 1],
