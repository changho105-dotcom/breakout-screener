...
const CONFIG = {
  ...
  breakoutLookback: 252,         // 돌파 판단 기준 기간 - 52주(전고점) 돌파로 변경 (거래일 기준 약 252일)
  fiftyTwoWeekLookback: 252,     // 52주 신고가 계산 기간
  nearHighThreshold: -0.20,      // "52주 신고가 근접" 판정 기준: 신고가 대비 -20% 이내
...
/**
 * 52주 신고가 대비 현재가 위치 (예전의 RS 자리를 대체)
 * 0 = 52주 신고가 갱신 중, 음수면 그만큼 신고가 대비 낮은 상태
...
/**
 * 돌파 여부 판단 - 52주 신고가(전고점) 돌파 기준
 * closes: 종가 배열, lookback(기본 52주=252거래일) 기간의 "당일 제외 최고가"를 오늘 종가가 넘었는지 확인
...
function evaluateStock({ ticker, name, sector, highs, closes, volumes, indexCloses, indexRegime, sectorAvgReturn, market }) {
  ...
  const highDistance = distanceFromFiftyTwoWeekHigh(highs, closes);
  const liquidityOk = passesLiquidity(closes, volumes, market);

  const checklist = {
    marketRegimeOk: indexRegime === 'STRONG_UP' || indexRegime === 'WEAK_UP',
    relativeStrengthOk: highDistance !== null && highDistance >= CONFIG.nearHighThreshold, // 52주 신고가 -3% 이내 근접
  ...
  return {
    ...
    relativeStrength: highDistance, // 필드명은 유지(프론트 호환), 의미는 "52주 신고가 대비 위치"
