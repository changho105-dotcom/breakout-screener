...
const CONFIG = {
  ...
  // ── VCP(변동성 수축 패턴) 근사 탐지 관련 ──
  vcpLookback: 63,                 // 베이스로 볼 기간 (약 3개월)
  vcpSegments: 3,                  // 몇 구간으로 나눠서 조정폭 축소 여부를 볼지
};
...
/**
 * 52주 저점 대비 현재가 상승률 (미너비니 기준: 최소 30% 이상 상승해 있어야 함)
 * lows: 저가 배열, closes: 종가 배열
 */
function riseFromFiftyTwoWeekLow(lows, closes, lookback = CONFIG.fiftyTwoWeekLowLookback) {
  if (lows.length < lookback || closes.length < lookback) return null;
  const low52w = Math.min(...lows.slice(-lookback));
  const lastClose = closes[closes.length - 1];
  if (!low52w) return null;
  return lastClose / low52w - 1;
}
...
/**
 * VCP(변동성 수축 패턴) 근사 탐지
 * ⚠️ 미너비니가 손으로 차트를 보며 판단하는 다중 파동 패턴을 완벽히 재현한 건 아니고,
 * "구간을 3등분해서 조정폭이 점점 좁아지고 있는지 + 거래량이 말라가는지"로 근사한 겁니다.
 * 정교한 VCP 인식은 이것보다 훨씬 복잡해서(파동 개수 가변, 각도 등) MVP 단계에서는 이 정도로 단순화했습니다.
 *
 * closes/volumes: 종가/거래량 배열, lookback: 베이스로 볼 기간(기본 약 3개월=63거래일)
 * 반환: { vcpOk, contractions } - contractions는 [1구간, 2구간, 3구간] 조정폭(%) (오래된→최근 순)
 */
function checkVCP(closes, volumes, lookback = CONFIG.vcpLookback, segments = CONFIG.vcpSegments) {
  if (closes.length < lookback + 1 || volumes.length < lookback + 1) return { vcpOk: null, contractions: null };

  const priceWindow = closes.slice(-(lookback + 1), -1); // 오늘 제외
  const volWindow = volumes.slice(-(lookback + 1), -1);
  const segLen = Math.floor(priceWindow.length / segments);
  if (segLen < 3) return { vcpOk: null, contractions: null }; // 세그먼트가 너무 짧으면 판단 불가

  const contractions = [];
  const volAvgs = [];
  for (let s = 0; s < segments; s++) {
    const start = s * segLen;
    const end = s === segments - 1 ? priceWindow.length : start + segLen;
    const seg = priceWindow.slice(start, end);
    const segVol = volWindow.slice(start, end);
    const segMax = Math.max(...seg);
    const segMin = Math.min(...seg);
    contractions.push(segMax > 0 ? (segMax - segMin) / segMax : 0);
    volAvgs.push(segVol.reduce((a, b) => a + b, 0) / segVol.length);
  }

  // 조정폭이 첫 구간보다 마지막 구간에서 확실히 좁아졌는지 (엄격한 매 구간 감소까지는 요구하지 않음)
  const contractingOk = contractions[contractions.length - 1] < contractions[0] * 0.7;
  // 거래량이 처음 구간보다 마지막 구간에서 줄어들었는지 (수급 소멸)
  const volumeDryUpOk = volAvgs[volAvgs.length - 1] < volAvgs[0];

  return { vcpOk: contractingOk && volumeDryUpOk, contractions };
}
...
function evaluateStock({ ticker, name, sector, highs, closes, lows, volumes, indexCloses, indexRegime, sectorAvgReturn, market, lookback, rsRatingPercentile }) {
  ...
  const lowRise = riseFromFiftyTwoWeekLow(lows, closes);
  const vcp = checkVCP(closes, volumes);
  ...
  const checklist = {
    ...
    ...(lowRise !== null ? { lowRiseOk: lowRise >= CONFIG.minRiseFromLowPct } : {}), // 52주 저점 대비 +30% 이상
    ...(vcp.vcpOk !== null ? { vcpOk: vcp.vcpOk } : {}), // VCP(변동성 수축) 근사 판정
  ...
  // 개별 종목 강도(%): 시장국면을 제외한 종목 고유 조건들 중 통과 비율
  const { marketRegimeOk, ...stockOnlyChecklist } = checklist;
  const stockCriteria = Object.values(stockOnlyChecklist);
  const stockStrengthPct = stockCriteria.length
    ? Math.round((stockCriteria.filter(Boolean).length / stockCriteria.length) * 100)
    : 0;
  ...
  return {
    ...
    lowRise, // 52주 저점 대비 상승률 (null이면 데이터 부족)
    rsRatingPercentile: rsRatingPercentile ?? null,
    vcpContractions: vcp.contractions, // 참고용 - 구간별 조정폭 배열
