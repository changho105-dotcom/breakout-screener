 ...
 *
 * 2026-08-06 대규모 검증(3개 시장 x 여러 기간 백테스트) 결과 반영:
 * - "신고가근접" 항목 삭제 - 전고점돌파(breakoutOk)에 이미 통과했다면 정의상 항상 자동으로 참이 되는
 *   죽은 조건이었음(3개 시장 전부 제거해도 백테스트 결과 완전히 동일하게 확인됨). relativeStrength
 *   수치 자체(고점대비 %)는 화면 표시용으로 계속 계산하되, 체크리스트 점수에서는 제외.
 * - "시장국면" 항목은 한국(KR)에서는 유지, 미국(US)에서는 제외 - 제거검증 결과 미국은 국면필터를
 *   빼는 쪽이 수익률·MDD 모두 일관되게 나았고, 한국은 반대로 크게 나빠졌음(시장별로 다르게 반영).
 */
function evaluateStock({ ticker, name, sector, highs, closes, lows, volumes, indexCloses, indexRegime, sectorAvgReturn, market, lookback, rsRatingPercentile }) {
  ...
  const checklist = {
    ...(market === 'KR' ? { marketRegimeOk: indexRegime === 'STRONG_UP' || indexRegime === 'WEAK_UP' } : {}),
