...
const CONFIG = {
  ...
  breakoutLookback: 20,          // 돌파 판단 기준 기간 (며칠간의 고점을 돌파했는지)
  rsLookback: 60,                // 상대강도 계산 기간
  rsTopPercentile: 0.8,          // 상대강도 상위 몇 % 안에 들어야 "주도주"로 볼지 (0.8 = 상위 20%)
...
/**
 * 상대강도 (RS) = 종목 수익률 - 지수 수익률 (rsLookback 기간)
 */
function relativeStrength(stockCloses, indexCloses, lookback = CONFIG.rsLookback) {
  if (stockCloses.length < lookback + 1 || indexCloses.length < lookback + 1) return null;
  const stockRet = stockCloses[stockCloses.length - 1] / stockCloses[stockCloses.length - 1 - lookback] - 1;
  const indexRet = indexCloses[indexCloses.length - 1] / indexCloses[indexCloses.length - 1 - lookback] - 1;
...
/**
 * 돌파 여부 판단
 * closes: 종가 배열, lookback 기간의 "당일 제외 최고가"를 오늘 종가가 넘었는지 확인
 ...
 */
function evaluateStock({ ticker, name, sector, highs, closes, volumes, indexCloses, indexRegime, sectorAvgReturn, market }) {
  const breakout = checkBreakout(highs, closes);
  const volRatio = volumeSurgeRatio(volumes);
  const rs = relativeStrength(closes, indexCloses);
  const liquidityOk = passesLiquidity(closes, volumes, market);

  const checklist = {
    marketRegimeOk: indexRegime === 'STRONG_UP' || indexRegime === 'WEAK_UP',
    relativeStrengthOk: rs !== null && rs > 0,
