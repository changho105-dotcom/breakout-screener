...
async function fetchStockList() {
  ...
  // 2026-08-29: res.ok 체크 없이 바로 .text()해서 csv-parse에 넘기면, 차단/오류 응답의
  // HTML·JSON 에러 본문이 "파싱은 되는데 이상한 종목 리스트"로 둔갑할 위험이 있음
  // (krSource.js에서 실제로 벌어졌던 사고와 같은 종류). 세 소스 모두 명시적으로 체크.
  for (const [res, url] of [[sp500Res, SP500_LIST_URL], [dowRes, DOW_LIST_URL], [ndxRes, NASDAQ100_LIST_URL]]) {
    if (!res.ok) throw new Error(`미국 종목리스트 응답 오류 (${url}): 상태코드 ${res.status}`);
  }
...
async function fetchOHLCVFromYahoo(symbol, days = 100) {
  const range = days > 500 ? '5y' : days > 200 ? '2y' : '6mo';
  ...
  for (let i = 0; i < (quote.close || []).length; i++) {
    ...
    dates.push(timestamps[i] ? new Date(timestamps[i] * 1000).toISOString().slice(0, 10) : null);
  }
